...
async fn test_change_username_happy_path() {
    ...
    let updated_user: User = assert_ok!(User::find(&mut conn, model.id).await);
...
async fn test_reject_prohibited_username_changes() {
    let (app, _, user) = TestApp::init().with_user().await;
    let mut conn = app.db_conn().await;
    let initial_model = User::find(&mut conn, user.as_model().id)
        .await
    ...
    let final_model = assert_ok!(User::find(&mut conn, user.as_model().id).await);
